                ...
                using (SqlCommand cmd = new SqlCommand("USP_EmiReminder", conn))
                {
                    ...
                    if (dt.Rows.Count > 0 && dt != null)
                    {
                        foreach (DataRow row in dt.Rows)
                        {
                            ...
                            double waiveoffamount = row["waive_off_amount"] != DBNull.Value ? Convert.ToDouble(row["waive_off_amount"]) : 0.0;
                            ...
                            double _loan_amount = row["principal_amount"] != DBNull.Value ? Convert.ToDouble(row["principal_amount"]) : 0.0;
                            double penal_charge = row["penal_charge"] != DBNull.Value ? Convert.ToDouble(row["penal_charge"]) : 0.0;
                            double penaldaycount = double.TryParse(row["penalty_days"]?.ToString(), out double tempPenal) ? tempPenal : 0;
                            double intrestperday = double.TryParse(row["interest_rate"]?.ToString(), out double tempInterest) ? tempInterest : 1;

                            double dpd_interest_amount = (_loan_amount * intrestperday / 100) * penaldaycount;
                            double till_day_interest_amount = (_loan_amount * intrestperday / 100) * till_date_interest_dpd;
                            double penal_amount = (_loan_amount * penal_charge / 100) * penaldaycount;

                            emailContentModel.amount = Math.Round((_loan_amount + _total_remaining_interest_due + till_day_interest_amount + penal_amount) - (waiveoffamount), 2);
